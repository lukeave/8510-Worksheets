...
> I really couldn't answer this one. I even made a plot to see if visualizing the numbers would help me understand why they are shorter today. But it didn't. It feels like I could be missing some historical context here?

(@) Filter the dataset to address this discrepancy and the recreate these statistics: 
```{r}
avg.words.sotu <- avg.words.sotu %>%
  left_join(y = meta, by = "president")

avg.words.sotu %>%
    ggplot(aes(year, avg_words, colour = sotu_type)) +
  geom_line() +
  geom_point()
```
