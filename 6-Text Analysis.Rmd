...
> meta <- as.data.frame(sotu_meta)
#here, we retrieve metadata for the State of the Union Addresses and store it as a dataframe.
file_paths <- sotu_dir(dir = "sotu_files")
sotu_texts <- readtext(file_paths)
sotu_whole <- 
  sotu_meta %>%  
  ...
  bind_cols(sotu_texts) %>% # combine with texts
  as_tibble() 
#here, we combine metadata and data to form the corpus object to be preprocessed and analyzed.
tidy_sotu <- sotu_whole %>%
  unnest_tokens(word, text) %>% 
  anti_join(stop_words)
#here, we preprocess the corpus and turn it into tidy text data through tokenization. 

> These steps are the first part of a workflow for a topic modeling that takes the tidy text data after preprocessing and unveils the abstract themes in the corpus, separating words into topics.
...
```
> I think the code above does not answer the question since it gives the average of word count per president. To answer who had the longest and shortest speech respectively, I would write the following code:

```{r}
test <- tidy_sotu %>%
  select(president, sotu_type, doc_id) %>%
  group_by(president, doc_id) %>% 
  summarize(count = n()) %>% 
  arrange(desc(count))
```

> Jimmy Carter made the longest address while Dwight D. Eisenhower made the shortest. 
...
```{r}
test2 <- tidy_sotu %>%
  select(sotu_type, doc_id) %>%
  group_by(sotu_type, doc_id) %>% 
  summarize(average = mean(n)) %>% 
  arrange(desc(average))
```


> Johm Adams' short SOTUS was a spoken speech while William Taft's long SOTUS was a written speech.

(@) Filter the dataset to address this discrepancy and the recreate these statistics: 
```{r}
vis.avg.word <- tidy_sotu %>%
  group_by(sotu_type, president, doc_id) %>%
  count(doc_id) %>%
  group_by(sotu_type, president) %>%
  summarize(average = mean(n)) %>% 
  arrange(desc(average))

plot <- ggplot(vis.avg.word, mapping = aes(x = president, y = average, fill = sotu_type)) +
  geom_bar(position = "dodge", stat = "identity") + coord_flip()
ggsave(plot, filename="myfigure.png")
``` 
...
> First, count() counts the amount of times each word is used by each president. Then, the code groups the result by president and uses mutate() to create a new column in the dataframe called "n_tot" containing the number of words in total in each document, per president. it also creates another colum called "term_freq" that containes the term frequency for each word. Then, it sorts out by the highest term frequency and it uses the funciton top_n() with n = 1, which retrieves the first observation in the dataframe for each president.
...
```{r}
stop_words_custom <- stop_words %>% add_row(word="america", lexicon="NA") %>% 
  add_row(word="american", lexicon="NA") %>% 
  add_row(word="americans", lexicon="NA") %>% 
  add_row(word="people", lexicon="NA")

obama.words <- tidy_sotu %>%
...
obama.words <- obama.words %>% bind_tf_idf(word, doc_id, n) 
head(obama.words)
...
```{r}
sotu_word_pairs %>% 
  filter(n == 125:175) %>%  # only word pairs that occur 200 or more times
  graph_from_data_frame() %>% #convert to graph
  ggraph(layout = "fr") + # place nodes according to the force-directed algorithm of Fruchterman and Reingold
  geom_edge_link(aes(edge_alpha = n, edge_width = n), edge_colour = "tomato") +
  geom_node_point(size = 5) +
...
metadata <- read.csv("https://github.com/regan008/8510-TextAnalysisData/blob/main/mb-metadata.csv")
```

(@) Add code chunks below and intersperse text to explain what you are doing and why. 
```{r}
mind.body <- readtext(file.path("~/Desktop/8510-Worksheets/txt"))

#mind.body.whole <- metadata %>%  
  #bind_cols(mind.body) %>% # combine with texts
  #as_tibble() # convert to tibble
#glimpse(mind.body.whole) 
#well that did not work since the metadata is useless and it has more observations than the text data itself (?).

#pre-processing: tokenization
tidy.mind.body <- mind.body %>%
  unnest_tokens(word, text) %>%
  anti_join(stop_words_custom) #removing the stop words
glimpse(tidy.mind.body)

tidy.mind.body %>%
  count(word, sort = TRUE) #%>% #raw count of words
 # as.data.frame()

tidy.mind.body %>%
  filter(word %in% c("future", "past")) %>% #searching for the occurrance of words future and past
  count(doc_id, word, sort = TRUE) #%>% 
  #as.data.frame()

mb.past.future <- tidy.mind.body %>%
  filter(word %in% c("future", "past")) %>%
  count(doc_id, word, sort = TRUE) %>%
  filter(grepl("1900", doc_id) | grepl("1901", doc_id) | grepl("1902", doc_id) | grepl("1903", doc_id) | grepl("1904", doc_id)) 

mb.past.future %>% 
  ggplot(aes(doc_id, n, fill = word)) +
